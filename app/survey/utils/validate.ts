 ...
 * @returns 未回答的必要题目列表
 */
export function checkNecessaryQuestions(answers: Answers, questions: Questions): UnansweredQuestion[] {
  // 将answers转换为统一格式(Map)
 ...
 * @returns 可见但未回答的问题列表
 */
export function getVisibleUnansweredQuestions(answers: Answers, questions: Questions): UnansweredQuestion[] {
  // 将answers转换为统一格式(Map)
  const answersMap = answers instanceof Map ? answers : new Map(Object.entries(answers).map(([key, value]) => [Number(key), value]));
